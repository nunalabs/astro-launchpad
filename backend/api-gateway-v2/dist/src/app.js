...
export async function createApp() {
    ...
    app.addHook('onRequest', async (request, reply) => {
        const { securityCheck, getClientIP, logSecurityEvent } = await import('./lib/security.js');
        // Run security checks
    ...
    app.addHook('onResponse', async (request, reply) => {
        ...
        // Record metrics
        recordHttpRequest(request.method, request.routeOptions?.url || request.url, reply.statusCode, duration, requestSize, responseSize);
    });
    ...
    app.addHook('preValidation', async (request, reply) => {
        ...
        if (!request.url.startsWith('/graphql'))
            return;
        const { validateQuery, checkRateLimitAdvanced, isExpensiveOperation, logSecurityEvent, } = await import('./lib/security.js');
        // Get query from body
    ...
    await app.register(mercurius, {
        ...
        errorFormatter: (execution, context) => {
            ...
            const operationName = context.reply?.request?.body?.operationName || 'unknown';
            ...
            const startTime = context.reply?.request?.startTime;
            if (startTime) {
