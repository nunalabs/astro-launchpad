...
function hasDeletedAtFilter(where: Record<string, unknown> | undefined): boolean {
  ...
  if (Array.isArray(where.AND)) return (where.AND as Record<string, unknown>[]).some(hasDeletedAtFilter)
  if (Array.isArray(where.OR)) return (where.OR as Record<string, unknown>[]).some(hasDeletedAtFilter)
  return false
}
 ...
 */
function addDeletedAtFilter(where: Record<string, unknown> | undefined): Record<string, unknown> {
  if (!where || Object.keys(where).length === 0) {
    return { deletedAt: null }
  }
  if (Array.isArray(where.AND)) {
    return { ...where, AND: [...(where.AND as unknown[]), { deletedAt: null }] }
  }
